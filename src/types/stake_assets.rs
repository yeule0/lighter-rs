...
impl TxInfo for StakeAssetsTx {
    fn get_tx_type(&self) -> u8 { TX_TYPE_L2_STAKE_ASSETS }
    ...
    fn validate(&self) -> Result<(), TxError> {
        ...
        if self.amount < MIN_STAKING_SHARES_TO_MINT_OR_BURN { return Err(TxError::StakeAmountTooLow); }
        if self.amount > MAX_STAKING_SHARES_TO_MINT_OR_BURN { return Err(TxError::StakeAmountTooHigh); }
    ...
    fn hash(&self, chain_id: u32) -> Result<Vec<u8>, TxError> {
        ...
        elems.push(GoldilocksField(TX_TYPE_L2_STAKE_ASSETS as u64));
