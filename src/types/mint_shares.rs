...
impl TxInfo for MintSharesTx {
    fn get_tx_type(&self) -> u8 { TX_TYPE_L2_MINT_SHARES }
    ...
    fn validate(&self) -> Result<(), TxError> {
        ...
        if self.share_amount < MIN_POOL_SHARES_TO_MINT_OR_BURN { return Err(TxError::MintShareAmountTooLow); }
        if self.share_amount > MAX_POOL_SHARES_TO_MINT_OR_BURN { return Err(TxError::MintShareAmountTooHigh); }
    ...
    fn hash(&self, chain_id: u32) -> Result<Vec<u8>, TxError> {
        ...
        elems.push(GoldilocksField(TX_TYPE_L2_MINT_SHARES as u64));
        ...
        elems.push(GoldilocksField(self.share_amount as u64));
