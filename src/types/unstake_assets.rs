...
impl TxInfo for UnstakeAssetsTx {
    fn get_tx_type(&self) -> u8 { TX_TYPE_L2_UNSTAKE_ASSETS }
    ...
    fn validate(&self) -> Result<(), TxError> {
        ...
        if self.amount < MIN_STAKING_SHARES_TO_MINT_OR_BURN { return Err(TxError::UnstakeAmountTooLow); }
        if self.amount > MAX_STAKING_SHARES_TO_MINT_OR_BURN { return Err(TxError::UnstakeAmountTooHigh); }
    ...
    fn hash(&self, chain_id: u32) -> Result<Vec<u8>, TxError> {
        ...
        elems.push(GoldilocksField(TX_TYPE_L2_UNSTAKE_ASSETS as u64));
        ...
        elems.push(GoldilocksField(self.staking_pool_index as u64));
        elems.push(GoldilocksField(self.amount as u64));
